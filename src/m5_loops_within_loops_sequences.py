...
def run_test_integers():
    ...
    # Test 2:
    expected = [8, 12, -1, 0, 30]
    answer = integers([(2.25, 8, 'Jessica'),
                       (12, 1.0, 'hi'),
                       [-1, 0, -.05, 'bonjour!'],
                       'quoi?',
                       [],
    ...
    # Test 3:
    expected = [8, 101, -101, 0]
    answer = integers([(2.25, 8, 'Jessica'), (101, -101, 0, 1.000, 4.321)])
    print('Expected is:', expected)
    print('Actual is:  ', answer)
    if expected == answer:
        print("SUCCESS")
...
def run_test_big_letters():
    ...
    # Test 2:
    expected = 'HITHERE'
    actual = big_letters([(), (2), ['O'], 'hi', 'oH', 'Is there any left?', ' ', 'TiHiEiRiE', '_', (5, 1)])
    ...
    # Test 3:
    expected = 'BIG'
    actual = big_letters([(1.0988, 'A'), 'BIG', 'letters', 'last9', '8', ('cAts', 9)])
    print("expected: ", expected)
    print("actual: ", actual)
    if expected == actual:
        print("SUCCESS")
...
def big_letters(sequence_of_sequences):
    ...
    for k in range(len(sequence_of_sequences)):
        if type(sequence_of_sequences[k]) is str:
            for j in range(len(sequence_of_sequences[k])):
                if str.isupper(sequence_of_sequences[k][j]):
